  ...
  <body class="d-flex flex-row justify-content-center">
    <div class="sidebar d-none d-lg-flex flex-column align-items-center">
      ...
      <div class="links d-flex flex-column justify-content-around">
        ...
        <a class="current-page" href="controller?acao=Listar&tipo=investimento">Investimentos</a>
    ...
    <div class="main-box d-flex flex-column justify-content-around">
      ...
      <div class="user-container col-12 d-flex flex-column justify-content-center align-items-center">
       <h2 class="username">Investimentos</h2>
      <div class="col-12 d-flex flex-row justify-content-end">
        	<a class="btn btn-light" href="/Financy/controller?acao=AbrirFormCadastro&tipo=investimento">Adicionar</a>
        </div>
            ...
            <table>
                <tr>
                 ...
                 <c:forEach items="${investimentos}" var="i">
                	<tr>
                		<td>${i.nome}</td>
                		<td>R$ <fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${i.valor}" /></td>
                		<td>
                			<fmt:formatDate value="${i.dataRealizacao}" pattern="dd/MM/yyyy"/>
                		</td>
                		<td>
                			<fmt:formatDate value="${i.dataConclusao}" pattern="dd/MM/yyyy"/>
                		</td>
                		<td>${i.tipoInvestimento}</td>
                		<td>${i.bancoCorretora}</td>
                		<td>
                			<c:url value="controller" var="link">
                				<c:param name="acao" value="AbrirFormAtualizar"></c:param>
                				<c:param name="tipo" value="investimento"></c:param>
                				<c:param name="codigoInvestimento" value="${i.cdInvestimento}"></c:param>
                			</c:url>
                			
                			<a class="btn btn-warning" href="${link}">Editar</a>
                		</td>
                		<td>
                			<button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#excluirModal" onclick="codigoExcluir.value = ${i.cdInvestimento}">
  								Excluir
							</button>
						</td>
                	 </tr>
